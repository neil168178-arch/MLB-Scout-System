...
with st.sidebar:
    ...
    st.markdown("---")
    data_mode = st.radio("選擇系統模式", ["📊 全聯盟一般數據", "🦄 Fantasy 夢幻棒球", "🔍 深度搜尋面板"], key="main_data_mode")
    st.markdown("---")
...
if not raw_data.empty:
    ...
    def color_rank_rows(row):
        ...
        id_cols = ['Player', 'Team', 'Position', 'Nickname']
    ...
    def inject_custom_css(primary_col, secondary_col):
        st.markdown(f"""
            ...
            [data-testid="stSidebar"] label, [data-testid="stSidebar"] p, [data-testid="stSidebar"] h1, [data-testid="stSidebar"] h2, [data-testid="stSidebar"] h3, [data-testid="stSidebar"] span {{ color: {secondary_col} !important; text-shadow: 0px 1px 3px rgba(0,0,0,0.6), 0px 0px 2px rgba(255,255,255,0.2) !important; font-weight: bold; }}
            ...
            [data-testid="stAppViewContainer"], .stApp {{ background-color: {hex_to_rgba(primary_col, 0.05)} !important; transition: background-color 0.5s ease; }}
            [data-testid="stHeader"] {{ background-color: transparent !important; }}
            [data-testid="stMetricValue"] {{ color: {primary_col} !important; text-shadow: 1px 1px 2px rgba(0,0,0,0.1); }}
            ...
            table.dataframe thead th {{ background-color: {primary_col}; color: white !important; font-size: {f_size(st.session_state.table_font_size, 0.9)} !important; position: sticky; top: 0; z-index: 10; box-shadow: 0 2px 2px -1px rgba(0,0,0,0.4); }}
            button[data-baseweb="tab"] p {{ font-size: {f_size(st.session_state.font_size, 0.9)} !important; font-weight: bold; }}
            @keyframes blink {{ 0% {{ opacity: 1; }} 50% {{ opacity: 0.4; }} 100% {{ opacity: 1; }} }}
            </style>
        """, unsafe_allow_html=True)

    default_primary, default_secondary = get_team_color("Los Angeles Dodgers")
    current_primary, current_secondary = default_primary, default_secondary
    
    inject_custom_css(current_primary, current_secondary)

    st.markdown(f"""
        <div style="text-align: center; margin-bottom: 20px;">
            <h1 style="color: {current_primary}; text-shadow: 1px 1px 3px rgba(0,0,0,0.15); font-weight: 900; margin: 0; padding: 0;"> MLB 數據系統 </h1>
            <div style="width: 120px; height: 5px; background-color: {current_secondary}; margin: 10px auto; border-radius: 3px; box-shadow: 0px 1px 2px rgba(0,0,0,0.2);"></div>
        </div>
    """, unsafe_allow_html=True)
    ...
    # 📌 模式 A：全聯盟一般數據
    # ==========================================
    if data_mode == "📊 全聯盟一般數據":
        data = raw_data.copy()
        
        # 🔥 修改 3：完美排除 Fantasy 分數進入整體評分與球探報告雷達
        scout_metrics_l = [m for m in global_metrics if m not in ['CYC', 'SLAM', 'E', 'Fantasy_Score', 'Fantasy_Pts']]
        ...
        with tab_rank:
            ...
            sorted_data = sorted_data.drop(columns=['Nickname'], errors='ignore')
            ...
            with st.spinner("全網大範圍撈取最新戰報中..."):
                ...
                if not recent_df.empty:
                    ...
                    if not recent_df.empty:
                        ...
                        cols_r = list(recent_df.columns)
                        if 'Position' in cols_r: cols_r.remove('Position')
                        if 'Nickname' in cols_r: cols_r.remove('Nickname')
                        cols_r.insert(2, 'Position')
                        recent_df = recent_df[cols_r]
                        
                        cmap = 'Reds' if p_type == '打者' else ('Blues_r' if sel_recent_m in ['ERA', 'WHIP', 'BB'] else 'Blues')
                        styled_recent = recent_df.style.apply(lambda x: [highlight_elite_stats(v, x.name, p_type) for v in x], axis=0).apply(color_rank_rows, axis=1).format(STYLER_FORMATS).background_gradient(subset=[sel_recent_m], cmap=cmap).hide(axis='index')
        ...
        with tab_scatter:
            ...
            fig = px.scatter(data, x=x_col, y=y_col, color="Team", hover_name="Player", hover_data=["Nickname"] if "Nickname" in data.columns else None, color_discrete_map={t: get_team_color(t)[0] for t in data['Team'].unique()})
        ...
        # 🔥 修改 1：拿掉全大聯盟，MVP 分流為真實賽制的 AL / NL
        with tab_mvp:
            st.subheader(f"👑 {year} 賽季 MVP 預測排行榜")
            col_mvp1, _ = st.columns([1, 2])
            sel_league_mvp = col_mvp1.radio("選擇聯盟", ["美國聯盟 (AL)", "國家聯盟 (NL)"], horizontal=True, key="league_mvp")
            
            with st.spinner("運算 MVP 積分中..."):
                mvp_df = data.copy()
                if not mvp_df.empty:
                    if sel_league_mvp == "美國聯盟 (AL)": mvp_df = mvp_df[mvp_df['Team'].isin(AL_TEAMS)]
                    elif sel_league_mvp == "國家聯盟 (NL)": mvp_df = mvp_df[mvp_df['Team'].isin(NL_TEAMS)]
                    ...
                    mvp_df = mvp_df.drop(columns=['Nickname'], errors='ignore')
                    mvp_top = mvp_df.sort_values('MVP_Index', ascending=False).head(15).reset_index(drop=True)
                    mvp_top.index += 1
                    
                    styled_mvp = mvp_top[keep_cols].style\
                        .apply(lambda x: [highlight_elite_stats(v, x.name, p_type) for v in x], axis=0)\
                        .apply(color_rank_rows, axis=1)\
                        .format(STYLER_FORMATS)\
                        .background_gradient(subset=['MVP_Index'], cmap='YlOrRd')\
                        .hide(axis='index')
                        
                    st.markdown(f"<div class='table-scroll-container'>{styled_mvp.to_html()}</div>", unsafe_allow_html=True)
        
        # 🔥 修改 1：拿掉全大聯盟，賽揚獎分流為真實賽制的 AL / NL
        if p_type == "投手" and tab_cy is not None:
            with tab_cy:
                st.subheader(f"🏆 {year} 賽季 賽揚獎 (Cy Young) 預測排行榜")
                col_cy1, _ = st.columns([1, 2])
                sel_league_cy = col_cy1.radio("選擇聯盟", ["美國聯盟 (AL)", "國家聯盟 (NL)"], horizontal=True, key="league_cy")
                
                with st.spinner("運算賽揚積分中..."):
                    cy_df = data.copy()
                    if not cy_df.empty:
                        if sel_league_cy == "美國聯盟 (AL)": cy_df = cy_df[cy_df['Team'].isin(AL_TEAMS)]
                        elif sel_league_cy == "國家聯盟 (NL)": cy_df = cy_df[cy_df['Team'].isin(NL_TEAMS)]
                        ...
                        cy_df = cy_df.drop(columns=['Nickname'], errors='ignore')
                        cy_top = cy_df.sort_values('Cy_Index', ascending=False).head(15).reset_index(drop=True)
                        cy_top.index += 1
                        
                        styled_cy = cy_top[keep_cols_cy].style\
                            .apply(lambda x: [highlight_elite_stats(v, x.name, p_type) for v in x], axis=0)\
                            .apply(color_rank_rows, axis=1)\
                            .format(STYLER_FORMATS)\
                            .background_gradient(subset=['Cy_Index'], cmap='Blues')\
                            .hide(axis='index')
    ...
    # ==========================================
        ...
        with tab_weekly:
            ...
            with st.spinner("掃描近七日全聯盟逐場日誌，精算 Fantasy 積分中..."):
                ...
                if not weekly_df.empty:
                    ...
                    if 'Position' in cols_w: cols_w.remove('Position')
                    cols_w.insert(2, 'Position')
                    weekly_df = weekly_df.drop(columns=['Nickname'], errors='ignore')
        ...
        with tab_season:
            ...
            # 🔥 修改 2：將 CYC 與 SLAM 從賽季累積打者的欄位中移除，使畫面更精簡
            fantasy_cols = ['Player', 'Team', 'Position', 'Fantasy_Score', 'R', 'H', '1B', '2B', '3B', 'HR', 'RBI', 'SB', 'BB', 'HBP', 'K', 'E'] if p_type == '打者' else ['Player', 'Team', 'Position', 'Fantasy_Score', 'W', 'L', 'SHO', 'SV', 'OUT', 'H', 'ER', 'HR', 'BB', 'HBP', 'K', 'WP', 'HLD', 'QS', 'BSV']
            fantasy_df = raw_data[fantasy_cols].copy()
            
            col_f1, col_f2 = st.columns([1, 1])
            sel_fantasy_pos = col_f1.selectbox("🛡️ 篩選守備位置 (Season Fantasy)", ["全部 (ALL)", "DH", "C", "1B", "2B", "3B", "SS", "LF", "CF", "RF"] if p_type == '打者' else ["全部 (ALL)", "SP", "RP", "CL"], index=0, key='league_fan_season_pos')
            sort_f_order = col_f2.radio("排序方式", ["由高到低", "由低到高"], index=0, horizontal=True, key="league_fan_season_order")
            ...
            fantasy_df = fantasy_df.sort_values(by='Fantasy_Score', ascending=(sort_f_order == "由低到高")).reset_index(drop=True)
    ...
    # 📌 模式 C：深度搜尋面板
    # ==========================================
    elif data_mode == "🔍 深度搜尋面板":
        tab_player, tab_team, tab_nickname = st.tabs(["🧑 球員個人面版", "🏟️ 球隊戰情室", "🎭 外號同好會"])
        
        with tab_player:
            target_profile = st.selectbox("🔍 搜尋球員 (進入專屬面版)", options=all_players, index=None, placeholder="點選或輸入名字", key="deep_search_player")
            if target_profile:
                p_prof = raw_data[raw_data['Player'] == target_profile].iloc[0]
                theme_team = p_prof['Team']
                t_colors = get_team_color(theme_team)
                inject_custom_css(t_colors[0], t_colors[1]) 
                ...
                st.markdown(f"<h2 style='color:{t_colors[0]}; border-bottom: 3px solid {t_colors[0]}; padding-bottom: 10px; display: flex; align-items: center;'>{logo_html} <span> {target_profile} <span style='font-size:0.55em; background-color:{t_colors[0]}; color:#FFFFFF; border: 1.5px solid {t_colors[1]}; padding:4px 10px; border-radius:20px; margin-left:10px; vertical-align:middle; text-shadow:none;'>{fun_nickname}</span> <span style='font-size:0.6em; color:{t_colors[1]}; margin-left:10px;'>({hand_info})</span> | {p_prof['Team']} - {p_prof['Position']}</span></h2>", unsafe_allow_html=True)
                ...
                if not gamelog_df.empty:
                    ...
                    def color_gamelog_rows(row):
                        ...
                        bg = hex_to_rgba(t_colors[0], 0.18) if is_home else hex_to_rgba(get_team_color(row['Opponent'])[0], 0.18)
                ...
                st.markdown(f'''
                <div style="background: linear-gradient(135deg, {t_colors[0]} 0%, {darken_color(t_colors[0], 0.6)} 100%); padding: 20px 30px; border-radius: 12px; color: white; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 15px rgba(0,0,0,0.15);">
                    <div>
                ...
                st.markdown("---")
                # 🔥 修改 3：完美排除 Fantasy 分數進入個人頁面的深度評估報告雷達中
                scout_metrics = [m for m in global_metrics if m not in ['CYC', 'SLAM', 'E', 'Fantasy_Score', 'Fantasy_Pts', 'Fan_Pts', 'Avg_Pts']]
                prs = {m: get_percentile(raw_data, m, p_prof[m], p_type) for m in scout_metrics}
                ...
                single_df = raw_data[raw_data['Player'] == target_profile].drop(columns=['Player_ID', 'Nickname'], errors='ignore')
                ...
                with st.spinner("載入生涯數據..."):
                    ...
                    if not career_df.empty:
                        ...
                        fig_career = px.line(career_df, x='Season', y=sel_career_metric, hover_data=['Team'], markers=True, color_discrete_sequence=[t_colors[0]])
                ...
                st.info("👈 請在上方搜尋框輸入或選擇球員，以開啟專屬深度數據面板。")
                
        with tab_team:
            target_team = st.selectbox("🏟️ 選擇球隊 (進入球隊戰情室)", options=all_teams, index=None, placeholder="選擇球隊", key="deep_search_team")
            if target_team:
                ...
                st.markdown(f"<h2 style='color:{t_colors[0]}; border-bottom: 3px solid {t_colors[0]}; padding-bottom: 10px; display: flex; align-items: center;'>{logo_html} <span>{theme_team} 專屬分析戰情室</span></h2>", unsafe_allow_html=True)
                    ...
                    if not ts_df.empty and theme_team in ts_df['Team'].values:
                        ...
                        def team_metric_card(label, val_str, rank):
                            ...
                            <div style="background-color: white; padding: 15px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); margin-bottom: 15px; border: 1px solid #e0e0e0; text-align: center;">
                                ...
                                <div style="font-size: {f_size(st.session_state.font_size, 1.8)}; font-weight: 900; color: {t_colors[0]};">{val_str}</div>
                ...
                with st.spinner("載入大聯盟名單與本季精確守位..."):
                    ...
                    if not roster_df.empty:
                        styled_roster = roster_df.style.map(lambda _: f'color: {t_colors[0]} !important; font-weight: 700 !important;').hide(axis='index')
                ...
                with st.spinner("掃描全組織醫療與傷兵報告中 (包含小聯盟與 60天 IL)..."):
                    ...
                    if not il_df.empty:
                        styled_il = il_df.style.apply(lambda row: ['color: #D32F2F !important; font-weight: 900 !important;' if col == '名單狀態 (Status)' else f'color: {t_colors[0]} !important; font-weight: 700 !important;' for col in row.index], axis=1).hide(axis='index')
                ...
                st.info("👈 請選擇球隊，以開啟專屬戰情室。")

        with tab_nickname:
            target_nickname = st.selectbox("🎭 依專屬外號篩選 (同類球員大集合)", options=all_nicknames, index=None, placeholder="選擇外號，列出所有同類型的球員", key="deep_search_nickname")
            if target_nickname:
                st.success(f"🎭 **外號同好會啟動**：以下為全聯盟被 AI 判定為【{target_nickname}】的同類球員！")
                
                nk_data = raw_data[raw_data['Nickname'] == target_nickname].copy()
                
                if p_type == "打者":
                    tab_n_rank, tab_n_recent, tab_n_radar = st.tabs(["📊 排名", "🔥 近況", "📈 雷達"])
                else:
                    tab_n_rank, tab_n_recent, tab_n_radar = st.tabs(["📊 排名", "🔥 近況", "📈 雷達"])

                with tab_n_rank:
                    # 🔥 修改 3：完美排除 Fantasy 分數進入外號同好會排名與評等
                    scout_metrics_l = [m for m in global_metrics if m not in ['CYC', 'SLAM', 'E', 'Fantasy_Score', 'Fantasy_Pts', 'Fan_Pts', 'Avg_Pts']]
                    nk_data['綜合分數'] = [round(sum(get_relative_grade(nk_data, m, row[m], p_type)[1] for m in scout_metrics_l)/len(scout_metrics_l), 3) for _, row in nk_data.iterrows()]
                    nk_data = nk_data.sort_values(by='綜合分數', ascending=False).reset_index(drop=True)
                    nk_data.insert(0, '同好會排名', nk_data.index + 1)
                    nk_data.insert(1, '綜合評級', nk_data['綜合分數'].apply(score_to_grade))
                    nk_data = nk_data.drop(columns=['綜合分數'])
                    
                    col_sort1, col_sort2 = st.columns([1, 2])
                    sortable_cols = [c for c in nk_data.columns if c not in ['Player', 'Player_ID', 'Team', 'Position', 'Nickname', '同好會排名', '綜合評級']]
                    sort_metric = col_sort1.selectbox("🔍 選擇重新排序指標", sortable_cols, index=sortable_cols.index('WAR') if 'WAR' in sortable_cols else 0, key="nk_rank_metric")
                    
                    lower_is_better_metrics = ['Chase%', 'Whiff%', 'GB%', 'K%'] if p_type == '打者' else ['ERA', 'xERA', 'WHIP', 'FIP', 'BA', 'xBA', 'BB%', 'HardHit%', 'Barrel%', 'Diff']
                    sort_order = col_sort2.radio("排序方式", ["由高到低", "由低到高"], index=1 if sort_metric in lower_is_better_metrics else 0, horizontal=True, key="nk_rank_order")
                    
                    sorted_data = nk_data.sort_values(by=sort_metric, ascending=(sort_order == "由低到高")).reset_index(drop=True)
                    sorted_data['同好會排名'] = sorted_data.index + 1
                ...
                with tab_n_recent:
                    ...
                    with st.spinner("全網大範圍撈取最新戰報中..."):
                        recent_df = fetch_recent_form_ranking(p_type)
                        if not recent_df.empty:
                            recent_df = recent_df[recent_df['Player'].isin(nk_data['Player'])].reset_index(drop=True)
                                
                            if not recent_df.empty:
                                recent_df['Position'] = recent_df['Player'].map(raw_data.set_index('Player')['Position'].to_dict()).fillna(recent_df['Position']).replace('Unknown', 'DH/PH')
                                
                                c_rm, c_rp = st.columns(2)
                                sel_recent_m = c_rm.selectbox("📊 選擇近況排序指標", ['OPS', 'AVG', 'OBP', 'SLG', 'HR', 'RBI', 'PA'] if p_type == '打者' else ['ERA', 'WHIP', 'K', 'BB', 'SV', 'IP'], index=0, key="nk_recent_m")
                                sel_recent_pos = c_rp.selectbox("🛡️ 篩選守備位置", ["全部 (ALL)", "DH", "C", "1B", "2B", "3B", "SS", "LF", "CF", "RF"] if p_type == '打者' else ["全部 (ALL)", "SP", "RP", "CL"], index=0, key="nk_recent_pos")
                                ...
                                if not recent_df.empty:
                                    ...
                                    if 'Nickname' in cols_r: cols_r.remove('Nickname')
                                    cols_r.insert(2, 'Position')
                                    recent_df['Nickname'] = recent_df['Player'].map(raw_data.set_index('Player')['Nickname'].to_dict()).fillna("")
                                    cols_r.insert(3, 'Nickname')
                                    recent_df = recent_df[cols_r]
                            ...
                            else: st.warning("⚠️ 這個外號的球員們在近期並沒有賽事表現。")
                        else: st.warning("⚠️ 目前抓取不到近況數據！")

                with tab_n_radar:
                    st.markdown("### 🎯 同好會內部雷達大 PK")
                    col_t1, col_t2 = st.columns(2)
                    players_list = nk_data['Player'].unique()
                    
                    if len(players_list) > 0:
                        idx2 = 1 if len(players_list) > 1 else 0
                        target1_rad = col_t1.selectbox("雷達圖主要目標", players_list, index=0, key='nk_radar_t1')
                        target2_rad = col_t2.selectbox("雷達圖比較對象", players_list, index=idx2, key='nk_radar_t2')
                        
                        p1_rad, p2_rad = nk_data[nk_data['Player'] == target1_rad].iloc[0], nk_data[nk_data['Player'] == target2_rad].iloc[0]
                        t1_colors_rad, t2_colors_rad = get_team_color(p1_rad['Team']), get_team_color(p2_rad['Team'])
                        p1_color_rad, p2_color_rad = t1_colors_rad[0], (t2_colors_rad[0] if t2_colors_rad[0] != t1_colors_rad[0] else t2_colors_rad[1])
                        ...
                        cb_cols = st.columns(6)
                        for i, m in enumerate(global_metrics):
                            if cb_cols[i % 6].checkbox(m, value=m in default_rad_metrics, key=f"cb_nk_rad_{m}"): selected_metrics.append(m)
                                
                        if selected_metrics:
                            res1, res2 = [get_percentile(nk_data, m, p1_rad[m], p_type) for m in selected_metrics], [get_percentile(nk_data, m, p2_rad[m], p_type) for m in selected_metrics]
                            fig = go.Figure()
                            fig.add_trace(go.Scatterpolar(r=res1, theta=selected_metrics, fill='toself', line_color=p1_color_rad, name=target1_rad))
                            if target1_rad != target2_rad: fig.add_trace(go.Scatterpolar(r=res2, theta=selected_metrics, fill='toself', line_color=p2_color_rad, name=target2_rad))
                            fig.update_layout(polar=dict(radialaxis=dict(range=[0, 100])), showlegend=True, height=600, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
                            st.plotly_chart(fig, width="stretch", config={'scrollZoom': True})
                    else: st.warning("此類別暫無球員可供比較")
            else:
                st.info("👈 請選擇外號來啟動過濾功能。")