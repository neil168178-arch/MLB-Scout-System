...
with st.sidebar:
    ...
    min_filter = st.number_input("設定本季 PA (打席) 下限", min_value=0, value=30, step=10) if p_type == "打者" else st.number_input("設定本季 IP (投球局數) 下限", min_value=0.0, value=10.0, step=5.0)
...
if not full_data.empty:
    if mode == 'player':
        ...
        logo_url, hand_info = get_team_logo_url(p_prof['Team']), fetch_player_handedness(p_prof['Player_ID'])
        st.markdown(f"<h2 style='color:{p_prof_color}; border-bottom: 3px solid {p_prof_color}; padding-bottom: 10px; display: flex; align-items: center;'>{'<img src=\"'+logo_url+'\" width=\"45\" style=\"margin-right: 12px;\">' if logo_url else ''}<span>⚾ {target_profile} <span style='font-size:0.6em; color:{p_prof_secondary};'>({hand_info})</span> | {p_prof['Team']} - {p_prof['Position']}</span></h2>", unsafe_allow_html=True)
        ...
        if p_type == '打者':
            m_cols = st.columns(9)
            for i, m in enumerate(['AB', 'R', 'H', 'RBI', 'HR', 'SB', 'BB', 'K']): m_cols[i].metric(m, format_metric(p_prof[m], m), delta=f"+{int(last_game[m])}" if last_game is not None and last_game[m]>0 else None, delta_color="inverse" if m=='K' else "normal")
            ...
            for i, m in enumerate(['H', 'R', 'ER', 'BB', 'K', 'PC', 'HR']): m_cols[i+1].metric(m, format_metric(p_prof[m], m), delta=f"+{int(last_game[m])}" if last_game is not None and last_game[m]>0 else None, delta_color="normal" if m=='K' else ("off" if m=='PC' else "inverse"))
        ...
        if not gamelog_df.empty:
            ...
            show_cols = ['Date', 'Opponent', '主/客', 'AB', 'R', 'H', 'RBI', 'HR', 'SB', 'BB', 'K', 'AVG'] if p_type == '打者' else ['Date', 'Opponent', '主/客', 'IP', 'H', 'R', 'ER', 'BB', 'K', 'PC', 'HR', 'WHIP', 'ERA']
            recent_5 = recent_5.rename(columns={'AVG (賽季打擊率走勢)': 'AVG', 'WHIP (賽季WHIP走勢)': 'WHIP', 'ERA (賽季防禦率走勢)': 'ERA'})
            
            def color_gamelog_rows(row):
                bg = hex_to_rgba(p_prof_color, 0.18) if "主場" in str(row['主/客']) else hex_to_rgba(get_team_color(row['Opponent'])[0], 0.18)
                return [f'background-color: {bg}; color: black; font-weight: 500;' for col in row.index]
                
            st.markdown(f"<div class='table-scroll-container'>{recent_5[show_cols].style.apply(color_gamelog_rows, axis=1).format(STYLER_FORMATS).hide(axis='index').to_html()}</div>", unsafe_allow_html=True)
        else: st.info("⚠️ 目前查無本賽季出賽紀錄。")
        ...
        <div style="background: linear-gradient(135deg, {p_prof_color} 0%, {darken_color(p_prof_color, 0.6)} 100%); padding: 20px 30px; border-radius: 12px; color: white; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 15px rgba(0,0,0,0.15);">
            <div><h4 style="margin: 0 0 5px 0; color: rgba(255,255,255,0.9); font-weight: 600;">本週 (近7天) Fantasy 總積分</h4><div style="font-size: {f_size(st.session_state.font_size, 3.0)}; font-weight: 900; line-height: 1; text-shadow: 2px 2px 4px rgba(0,0,0,0.2);">{weekly_score:,.0f} <span style="font-size: 0.4em; opacity: 0.8; font-weight: normal; text-shadow: none;">pts</span></div></div>
            <div style="text-align: right; opacity: 0.85; font-size: {f_size(st.session_state.font_size, 0.9)}; line-height: 1.6;">✓ 包含近 7 天的所有出賽紀錄<br>✓ 精確結算單場 CYC / SLAM / QS 等進階成就</div>
        </div>
        ''', unsafe_allow_html=True)
        
        if not recent_7d.empty:
            show_cols_f = ['Date', 'Opponent', '主/客', 'Fantasy_Pts', 'AB', 'R', 'H', 'RBI', 'HR', 'SB', 'BB', 'K'] if p_type == '打者' else ['Date', 'Opponent', '主/客', 'Fantasy_Pts', 'IP', 'H', 'R', 'ER', 'BB', 'K', 'PC', 'HR']
            ...
            st.markdown(f"<div class='table-scroll-container'>{recent_7d[show_cols_f].style.apply(color_recent_7d, axis=1).format(STYLER_FORMATS).hide(axis='index').to_html()}</div>", unsafe_allow_html=True)
        else:
        ...
        strengths, weaknesses = [item for item in sorted_prs if item[1] >= 75][:4], sorted([item for item in sorted_prs if item[1] <= 35][-4:], key=lambda x: x[1])
        ...
        st.info(f"**{generate_scout_conclusion(prs, p_prof, p_type)}** | 評級請點擊左側欄搜尋框右側 ✕ 回到全聯盟綜合排名查看")

        st.markdown("---")
        st.markdown("### 📊 本季單人完整進階數據表")
        styled_single = full_data[full_data['Player'] == target_profile].drop(columns=['Player_ID'], errors='ignore').style.apply(lambda x: [highlight_elite_stats(v, x.name, p_type) for v in x], axis=0).format(STYLER_FORMATS).hide(axis='index')
        ...
        with st.spinner("載入 Savant 進階對戰數據..."):
            ...
            if not platoon_df.empty: st.markdown(f"<div class='table-scroll-container' style='max-height: none;'>{platoon_df.style.format(STYLER_FORMATS).hide(axis='index').to_html()}</div>", unsafe_allow_html=True)
        ...
        with st.spinner("載入主客場數據..."):
            ...
            if not ha_df.empty: st.markdown(f"<div class='table-scroll-container' style='max-height: none;'>{ha_df.style.format(STYLER_FORMATS).hide(axis='index').to_html()}</div>", unsafe_allow_html=True)
        ...
        with st.spinner("載入生涯數據..."):
            ...
            if not career_df.empty:
                ...
                sel_career_metric = col_c_sel.selectbox("選擇生涯指標", career_metrics, index=career_metrics.index('OPS') if p_type == '打者' and 'OPS' in career_metrics else (career_metrics.index('ERA') if p_type == '投手' and 'ERA' in career_metrics else 0))
        ...
        st.markdown(f"<h2 style='color:{p_prof_color}; border-bottom: 3px solid {p_prof_color}; padding-bottom: 10px; display: flex; align-items: center;'>{'<img src=\"'+logo_url+'\" width=\"55\" style=\"margin-right: 15px; filter: drop-shadow(0px 2px 4px rgba(0,0,0,0.2));\">' if logo_url else ''}<span>{theme_team} 專屬分析戰情室</span></h2>", unsafe_allow_html=True)
        ...
        with st.spinner("載入 30 隊排行榜數據中..."):
            ...
            if not ts_df.empty and theme_team in ts_df['Team'].values:
                ...
                def team_metric_card(label, val_str, rank): return f'<div style="background-color: white; padding: 15px; border-radius: 8px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); margin-bottom: 15px; border: 1px solid #e0e0e0; text-align: center;"><div style="font-size: {f_size(st.session_state.font_size, 0.9)}; color: #555; margin-bottom: 5px; font-weight: bold;">{label}</div><div style="font-size: {f_size(st.session_state.font_size, 1.8)}; font-weight: 900; color: {p_prof_color};">{val_str}</div><div style="font-size: {f_size(st.session_state.font_size, 0.85)}; color: {"#00E676" if rank <= 15 else "#A9A9A9"}; font-weight: bold; margin-top: 5px;">聯盟第 {int(rank)} 名</div></div>'
        ...
        with st.spinner("載入近期戰況..."):
            ...
            if not recent_games_df.empty:
                styled_recent = recent_games_df.style.map(lambda x: f"color: white; background-color: {'#4CAF50' if x == 'W' else '#F44336'}; font-weight: bold; text-align: center;" if x in ['W', 'L'] else '', subset=['勝負 (Result)']).map(lambda val: f'color: {get_team_color(val)[0]} !important; font-weight: 900 !important;', subset=['對手 (Opponent)']).hide(axis='index')
        ...
        with st.spinner("載入大聯盟名單與本季精確守位..."):
            ...
            if not roster_df.empty: st.markdown(f"<div class='table-scroll-container' style='max-height: 800px;'>{roster_df.style.map(lambda _: f'color: {p_prof_color} !important; font-weight: 700 !important;').hide(axis='index').to_html()}</div>", unsafe_allow_html=True)
        ...
        with st.spinner("掃描全組織醫療與傷兵報告中 (包含小聯盟)..."):
            ...
            if not il_df.empty: st.markdown(f"<div class='table-scroll-container' style='max-height: 400px;'>{il_df.style.apply(lambda row: ['color: #D32F2F !important; font-weight: 900 !important;' if col == '名單狀態 (Status)' else f'color: {p_prof_color} !important; font-weight: 700 !important;' for col in row.index], axis=1).hide(axis='index').to_html()}</div>", unsafe_allow_html=True)
            else: st.success(f"✅ {theme_team} 目前非常健康，查無重大傷兵！")
        ...
        if p_type == "打者": tab_rank, tab_recent, tab_fantasy, tab_radar, tab_scatter, tab_h2h, tab_predict, tab_mvp, tab_milb = st.tabs(["📊 排名", "🔥 近況", "🦄 Fantasy", "📈 雷達", "🌌 散佈", "⚖️ 對決", "🔮 預測", "👑 MVP", "🌱 MiLB"]); tab_cy = None
        else: tab_rank, tab_recent, tab_fantasy, tab_radar, tab_scatter, tab_h2h, tab_predict, tab_mvp, tab_cy, tab_milb = st.tabs(["📊 排名", "🔥 近況", "🦄 Fantasy", "📈 雷達", "🌌 散佈", "⚖️ 對決", "🔮 預測", "👑 MVP", "🏆 賽揚", "🌱 MiLB"])
        ...
        with tab_rank:
            ...
            sort_metric = col_sort1.selectbox("🔍 選擇重新排序指標", sortable_cols, index=sortable_cols.index('WAR') if 'WAR' in sortable_cols else 0)
            
            lower_is_better_metrics = ['Chase%', 'Whiff%', 'GB%', 'K%'] if p_type == '打者' else ['ERA', 'xERA', 'WHIP', 'FIP', 'BA', 'xBA', 'BB%', 'HardHit%', 'Barrel%', 'Diff']
            sort_order = col_sort2.radio("排序方式", ["由高到低", "由低到高"], index=1 if sort_metric in lower_is_better_metrics else 0, horizontal=True)
            ...
            st.markdown(f"<div class='table-scroll-container'>{sorted_data.drop(columns=['Player_ID'], errors='ignore').style.apply(lambda x: [highlight_elite_stats(v, x.name, p_type) for v in x], axis=0).map(style_grade, subset=['綜合評級']).format(STYLER_FORMATS).hide(axis='index').to_html()}</div>", unsafe_allow_html=True)
            
        with tab_recent:
            st.markdown(f"### 🔥 {p_type}近況火熱排行榜")
            col_filt1, col_filt2 = st.columns([1, 2])
            recent_min_filter = col_filt1.slider("設定近況最少打席 (PA) 門檻", min_value=1, max_value=50, value=10, step=1) if p_type == '打者' else col_filt1.slider("設定近況最少投球局數 (IP) 門檻", min_value=1.0, max_value=30.0, value=3.0, step=0.5)
            col_filt2.caption(f"<br>以大數據掃描近期 14 天賽事，當前篩選門檻：至少 **{recent_min_filter}** {'打席' if p_type == '打者' else '局' }。", unsafe_allow_html=True)
                
            with st.spinner("全網大範圍撈取最新戰報中..."):
                recent_df = fetch_recent_form_ranking(p_type)
                if not recent_df.empty:
                    recent_df = recent_df[recent_df['PA'] >= recent_min_filter].copy() if p_type == '打者' else recent_df[recent_df['IP_calc'] >= recent_min_filter].copy().drop(columns=['IP_calc'])
                ...
                if not recent_df.empty:
                    ...
                    sel_recent_m = c_rm.selectbox("📊 選擇近況排序指標", ['OPS', 'AVG', 'OBP', 'SLG', 'HR', 'RBI', 'PA'] if p_type == '打者' else ['ERA', 'WHIP', 'K', 'BB', 'SV', 'IP'], index=0)
                    sel_recent_pos = c_rp.selectbox("🛡️ 篩選守備位置", ["全部 (ALL)", "DH", "C", "1B", "2B", "3B", "SS", "LF", "CF", "RF"] if p_type == '打者' else ["全部 (ALL)", "SP", "RP", "CL"], index=0)
                    
                    if sel_recent_pos != "全部 (ALL)": recent_df = recent_df[recent_df['Position'].astype(str).apply(lambda x: sel_recent_pos in [p.strip() for p in x.split(',')])]
                    ...
                    if not recent_df.empty:
                        ...
                        st.markdown(f"<div class='table-scroll-container'>{recent_df.style.apply(lambda row: [f'color: {get_team_color(row['Team'])[0]} !important; font-weight: 900 !important;' if col in ['Player', 'Team', 'Position'] else '' for col in row.index], axis=1).format(STYLER_FORMATS).background_gradient(subset=[sel_recent_m], cmap=cmap).hide(axis='index').to_html()}</div>", unsafe_allow_html=True)
                    else: st.warning("⚠️ 目前抓取不到符合此【守備位置】的近況數據。")
                else: st.warning("⚠️ 目前抓取不到符合此【局數/打席門檻】的近況數據，請嘗試往左調低拉條！")

        with tab_fantasy:
            st.markdown(f"### 🦄 Fantasy {p_type} 專屬結算面板")
            fan_mode = st.radio("選擇統計區間與賽制", ["🔥 近七日狀態結算排行 (包含 SLAM/QS)", "📊 本季累積數據篩選 (Season)"], horizontal=True)
            
            if fan_mode == "🔥 近七日狀態結算排行 (包含 SLAM/QS)":
                ...
                with st.spinner("掃描近七日全聯盟逐場日誌，精算 Fantasy 積分中..."):
                    ...
                    if not weekly_df.empty:
                        ...
                        sel_week_pos = col_w1.selectbox("🛡️ 篩選本週守備位置", ["全部 (ALL)", "DH", "C", "1B", "2B", "3B", "SS", "LF", "CF", "RF"] if p_type == '打者' else ["全部 (ALL)", "SP", "RP", "CL"], index=0)
                        
                        if sel_week_pos != "全部 (ALL)": weekly_df = weekly_df[weekly_df['Position'].astype(str).apply(lambda x: sel_week_pos in [p.strip() for p in x.split(',')])]
                        weekly_df.index += 1
                        st.markdown(f"<div class='table-scroll-container'>{weekly_df.style.apply(lambda row: [f'color: black !important; font-weight: 900 !important; font-size: 1.15em;' if col in ['Fan_Pts', 'Avg_Pts'] else (f'color: {get_team_color(row['Team'])[0]} !important; font-weight: 900 !important;' if col in ['Player', 'Team', 'Position'] else '') for col in row.index], axis=1).format(STYLER_FORMATS).hide(axis='index').to_html()}</div>", unsafe_allow_html=True)
                    else: st.warning("⚠️ 查無近七日比賽資料（目前可能處於休賽季）。")
            else:
                st.caption("完整提取夢幻棒球常用的累積計分項目！" if p_type == '投手' else "完整提取夢幻棒球常用的計分項目 (因全賽季運算量大，此區不計算 CYC/SLAM，請切換至「近七日結算排行」查看進階成就)。")
                ...
                sort_f_metric = col_f1.selectbox("📊 選擇排序指標 (Season Fantasy)", fantasy_cols[3:], index=0, key='fantasy_sort')
                sel_fantasy_pos = col_f2.selectbox("🛡️ 篩選守備位置 (Season Fantasy)", ["全部 (ALL)", "DH", "C", "1B", "2B", "3B", "SS", "LF", "CF", "RF"] if p_type == '打者' else ["全部 (ALL)", "SP", "RP", "CL"], index=0, key='fantasy_pos')
                
                if sel_fantasy_pos != "全部 (ALL)": fantasy_df = fantasy_df[fantasy_df['Position'].astype(str).apply(lambda x: sel_fantasy_pos in [p.strip() for p in x.split(',')])]
                if sort_f_metric not in ['CYC', 'SLAM']: fantasy_df = fantasy_df.sort_values(by=sort_f_metric, ascending=True if sort_f_metric in ['L', 'E', 'ER', 'H', 'BB', 'BSV', 'WP'] else False).reset_index(drop=True)
                    
                if not fantasy_df.empty:
                    fantasy_df.index += 1
                    st.markdown(f"<div class='table-scroll-container'>{fantasy_df.style.apply(lambda row: [f'color: black !important; font-weight: 900 !important; font-size: 1.1em;' if col == 'Fantasy_Score' else (f'color: {get_team_color(row['Team'])[0]} !important; font-weight: 900 !important;' if col in ['Player', 'Team', 'Position'] else '') for col in row.index], axis=1).format(STYLER_FORMATS).hide(axis='index').to_html()}</div>", unsafe_allow_html=True)
                else: st.warning("⚠️ 目前抓取不到符合此【守備位置】的 Fantasy 數據。")

        with tab_radar:
            st.markdown("### 🎯 選擇雷達圖比較目標")
            col_t1, col_t2 = st.columns(2)
            target1_rad, target2_rad = col_t1.selectbox("雷達圖主要目標", data['Player'].unique(), key='radar_t1'), col_t2.selectbox("雷達圖比較對象", data['Player'].unique(), key='radar_t2')
        ...
        with tab_scatter:
            ...
            x_col = col_sx.selectbox("X 軸", plot_metrics, index=plot_metrics.index('WAR') if 'WAR' in plot_metrics else 0)
            y_col = col_sy.selectbox("Y 軸", plot_metrics, index=plot_metrics.index('wRC+') if 'wRC+' in plot_metrics else (plot_metrics.index('Barrel%') if 'Barrel%' in plot_metrics else 1))
        ...
        with tab_h2h:
            ...
            h2h_t1, h2h_t2 = col_h1.selectbox("對決主要目標", data['Player'].unique(), key='h2h_t1'), col_h2.selectbox("對決比較對象", data['Player'].unique(), key='h2h_t2')
        ...
        with tab_predict:
            ...
            selected_game = None
            if schedule: selected_game = next(g for g in schedule if g['matchup'] == col_g.selectbox("選擇預測賽事", [g['matchup'] for g in schedule]))
            else: col_g.warning("該日無賽事或尚未公佈")
            ...
            if selected_game:
                ...
                with st.spinner("加載預測引擎數據與球隊近況..."):
                    ...
                    away_t_id, home_t_id = MLB_TEAM_IDS.get(away_t), MLB_TEAM_IDS.get(home_t)
                    
                    # 💡 呼叫新的 API：抓取近一週 OPS 與牛棚 ERA
                    away_weekly_ops, away_weekly_bp_era = fetch_team_weekly_ops_and_bp_era(away_t_id, target_date.strftime("%Y-%m-%d"))
                    home_weekly_ops, home_weekly_bp_era = fetch_team_weekly_ops_and_bp_era(home_t_id, target_date.strftime("%Y-%m-%d"))
                    ...
                    away_form, home_form = fetch_team_recent_form(away_t_id, target_date.strftime("%Y-%m-%d")), fetch_team_recent_form(home_t_id, target_date.strftime("%Y-%m-%d"))
                    ...
                    # 💡 升級預測引擎公式：將近一週 OPS 與近一週牛棚 ERA 的權重納入計算
                    away_strength = (away_ops * 60) + (away_weekly_ops * 40) + (max(0, 5 - away_p_era) * 10 + (ap_stats['WAR'].sum() if not ap_stats.empty else 0.0) * 5) + (max(0, 5 - away_weekly_bp_era) * 8) - (away_bp_pitches * 0.05) + (away_momentum * 1.5)
                    home_strength = (home_ops * 60) + (home_weekly_ops * 40) + (max(0, 5 - home_p_era) * 10 + (hp_stats['WAR'].sum() if not hp_stats.empty else 0.0) * 5) + (max(0, 5 - home_weekly_bp_era) * 8) - (home_bp_pitches * 0.05) + 3.0 + (home_momentum * 1.5)
                    ...
                    # 💡 增加 2 條能量條，並套用反轉參數 lower_is_better=True 給 ERA 與消耗球數
                    st.markdown(draw_comparison_bar(f"先發投手 ERA ({METRIC_TW.get('ERA')})", away_p_era, home_p_era, lower_is_better=True), unsafe_allow_html=True)
                    st.markdown(draw_comparison_bar(f"打線整體 OPS ({METRIC_TW.get('OPS')})", away_ops, home_ops), unsafe_allow_html=True)
                    st.markdown(draw_comparison_bar("近一週打線 OPS (近期狀態)", away_weekly_ops, home_weekly_ops), unsafe_allow_html=True)
                    st.markdown(draw_comparison_bar("近一週牛棚 ERA (近期壓制力)", away_weekly_bp_era, home_weekly_bp_era, lower_is_better=True), unsafe_allow_html=True)
                    st.markdown(draw_comparison_bar("近兩日牛棚消耗球數 (疲勞度)", away_bp_pitches, home_bp_pitches, is_int=True, lower_is_better=True), unsafe_allow_html=True)
                    ...
                    reasoning.append(f"🔥 **近期打線狀態**：{'主' if home_weekly_ops > away_weekly_ops else '客'}隊近一週 OPS ({max(home_weekly_ops, away_weekly_ops):.3f}) 手感較火熱。")
                    reasoning.append(f"🛡️ **近期牛棚壓制力**：{'主' if home_weekly_bp_era < away_weekly_bp_era else '客'}隊牛棚近一週 ERA ({min(home_weekly_bp_era, away_weekly_bp_era):.2f}) 後援較穩定。")
                    if home_bp_pitches > 80: reasoning.append(f"⚠️ <span style='color:#FF4444;'>**疲勞警告**</span>：主隊牛棚近期消耗巨大，可能在比賽後半段崩盤！")
                    if away_bp_pitches > 80: reasoning.append(f"⚠️ <span style='color:#FF4444;'>**疲勞警告**</span>：客隊牛棚近期消耗巨大，可能在比賽後半段崩盤！")
                    
                    with st.expander("🧠 點擊查看預測運算邏輯", expanded=True):
        ...
        with tab_milb:
            ...
            with st.spinner(f"撈取 {milb_level} {p_type} 數據中..."):
                ...
                if not milb_df.empty:
                    ...
                    sel_team = col_t.selectbox("選擇大聯盟母隊", ["全聯盟"] + sorted(milb_df['大聯盟母隊 (MLB Team)'].unique()))
                    sort_cols = [c for c in milb_df.columns if c not in ['球員 (Player)', '大聯盟母隊 (MLB Team)']]
                    sel_sort = col_s.selectbox("自訂排序指標", sort_cols, index=sort_cols.index('OPS') if p_type == '打者' and 'OPS' in sort_cols else (sort_cols.index('ERA') if p_type == '投手' and 'ERA' in sort_cols else 0))
                    lower_is_better_milb = ['ERA', 'WHIP', 'L'] if p_type == '投手' else []
                    sel_order = col_o.radio("排序方式 ", ["由高到低", "由低到高"], index=1 if sel_sort in lower_is_better_milb else 0, horizontal=True)
